...
```
the last page of app, first determine if guest, if so, not to call profile read on api. at the buttom of view, button show login if come from guest else showing logout and both of them will redirect to login page, logout first clear user data before redirection. if member, here we can update our profile by PATCH requests, after user enter new info on action sheet, update info on view and pass the PATCH request, if response is positive, show another alert sheet tell the user info update succeed.
